...
public class GaldrJsonSerializerGenerator : IIncrementalGenerator
{
    ...
    private static TypeInfo GetTypeInfoFromDeclaration(GeneratorSyntaxContext context)
    {
        ...
        // Check if the type has a serializable attribute
        bool hasAttribute = typeSymbol.GetAttributes().Any(attr =>
            ...
            attr.AttributeClass?.Name == "GaldrDbCollection" ||
            attr.AttributeClass?.Name == "GaldrDbCollectionAttribute" ||
            attr.AttributeClass?.Name == "GaldrDbProjection" ||
            attr.AttributeClass?.Name == "GaldrDbProjectionAttribute");
    ...
    private static void GenerateSerializerImplementation(IndentedStringBuilder builder, List<TypeInfo> types, TypeMetadataCache metadataCache)
    {
        ...
        using (builder.Block("internal class GeneratedJsonSerializer : IGaldrJsonTypeSerializer"))
        {
            ...
            // Serialize method - uses cached writer, delegates to SerializeTo
            using (builder.Block("public string Serialize(object value, Type type, GaldrJsonOptions options)"))
            {
                builder.AppendLine("Utf8JsonWriter writer = global::GaldrJson.Utf8JsonWriterCache.RentWriter(options.WriteIndented, out global::GaldrJson.GaldrBufferWriter bufferWriter);");
                builder.AppendLine("SerializeTo(writer, value, type, options);");
                builder.AppendLine("return System.Text.Encoding.UTF8.GetString(bufferWriter.WrittenSpan);");
            }

            builder.AppendLine();

            // SerializeTo method - core implementation
            using (builder.Block("public void SerializeTo(Utf8JsonWriter writer, object value, Type type, GaldrJsonOptions options)"))
            {
                ...
                builder.AppendLine("ReferenceTracker tracker = options.DetectCycles ? ReferenceTracker.Rent() : null;");
                builder.AppendLine();
