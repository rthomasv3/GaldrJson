            ...
            using (builder.Block("internal static object ReadWithConverter(ref Utf8JsonReader reader, Type type, JsonSerializerOptions options)"))
            {
                using (builder.Block("switch (type)"))
                {
                    foreach (TypeInfo type in types)
                    {
                        ...
                        builder.AppendLine($"case Type t1 when t1 == typeof(global::System.Collections.Generic.List<{type.FullName}>):");
                        builder.AppendLine($"case Type t2 when t2 == typeof(global::System.Collections.Generic.IEnumerable<{type.FullName}>):");
                        builder.AppendLine($"case Type t3 when t3 == typeof(global::System.Collections.Generic.IList<{type.FullName}>):");
                        builder.AppendLine($"case Type t4 when t4 == typeof(global::System.Collections.Generic.ICollection<{type.FullName}>):");
                        using (builder.Indent())
            ...
            using (builder.Block("internal static void WriteWithConverter(Utf8JsonWriter writer, object value, Type type, JsonSerializerOptions options, ReferenceTracker tracker)"))
            {
                using (builder.Block("switch (type)"))
                {
                    foreach (TypeInfo type in types)
                    {
                        ...
                        builder.AppendLine($"case Type t when t == typeof(global::System.Collections.Generic.List<{type.FullName}>):");
                        using (builder.Indent())
                        {
                        ...
                        builder.AppendLine($"case Type t1 when t1 == typeof(global::System.Collections.Generic.IEnumerable<{type.FullName}>):");
                        builder.AppendLine($"case Type t2 when t2 == typeof(global::System.Collections.Generic.IList<{type.FullName}>):");
                        builder.AppendLine($"case Type t3 when t3 == typeof(global::System.Collections.Generic.ICollection<{type.FullName}>):");
                        builder.AppendLine($"case Type t4 when t4 == typeof({type.FullName}[]):");
                        using (builder.Indent())
                        {
                ...
                using (builder.Block("try"))
                {
                    ...
                    using (builder.Block("switch (type)"))
                    {
                        foreach (TypeInfo type in types)
                        {
                            ...
                            builder.AppendLine($"case Type t3 when t3 == typeof(global::System.Collections.Generic.IList<{type.FullName}>):");
                            builder.AppendLine($"case Type t4 when t4 == typeof(global::System.Collections.Generic.ICollection<{type.FullName}>):");
                            builder.AppendLine($"case Type t5 when t5 == typeof({type.FullName}[]):");
                            using (builder.Indent())
                            {
